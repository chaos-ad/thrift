...
class t_cpp_generator : public t_oop_generator {
 public:
    ...
    : t_oop_generator(program)
  {
    ...
    gen_erl_nif_ = (iter != parsed_options.end());
  ...
  void generate_struct_erl_writer(t_struct* tstruct);
...
void t_cpp_generator::init_generator() {
  ...
  if (gen_erl_nif_) {
    string f_erl_nif_h_file = get_out_dir()+program_name_+"_erl.h";
    string f_erl_nif_c_file = get_out_dir()+program_name_+"_erl.cpp";
    f_erl_nif_c_.open(f_erl_nif_c_file.c_str());
    f_erl_nif_h_.open(f_erl_nif_h_file.c_str());
  }
  ...
  f_erl_nif_c_  << autogen_comment();
  f_erl_nif_h_  << autogen_comment();
  ...
  f_types_ <<
    ...
    "#include <transport/TTransport.h>" << endl << endl;

  f_erl_nif_h_ << "#include <ErlUtils.h>" << endl;
  ...
  for (size_t i = 0; i < includes.size(); ++i) {
    ...
    f_erl_nif_h_ <<
      "#include \"" << get_include_prefix(*(includes[i])) <<
      includes[i]->get_name() << "_erl.h\"" << endl;
  }
  f_types_ << endl;
  ...
  f_erl_nif_c_ <<
    ...
    f_erl_nif_c_ << "using namespace ::apache::thrift::erl_helpers;" << endl << endl;
...
void t_cpp_generator::generate_enum(t_enum* tenum) {
  ...
  */
  ...
  if(gen_erl_nif_) {
    std::string name = gen_pure_enums_ ? tenum->get_name() : tenum->get_name() + "::type";
    ...
    indent_up();
    f_erl_nif_c_ << indent() << "return static_cast<" << name << ">( read_value<int32_t>(env, term) );" << endl;
    indent_down();
    f_erl_nif_c_ << indent() << "}" << endl << endl;

    f_erl_nif_h_ << indent() << "ERL_NIF_TERM write_value(ErlNifEnv* env, " << name << " value);" << endl << endl;
    f_erl_nif_c_ << indent() << "ERL_NIF_TERM write_value(ErlNifEnv* env, " << name << " value) {" << endl;
    indent_up();
    f_erl_nif_c_ << indent() << "return write_value(env, static_cast<int32_t>(value));" << endl;
    indent_down();
    f_erl_nif_c_ << indent() << "}" << endl << endl;
  }
}
...
void t_cpp_generator::generate_struct_writer(ofstream& out,
  ...
  if(gen_erl_nif_) {
    generate_struct_erl_writer(tstruct);
  }
}
...
void t_cpp_generator::generate_struct_erl_writer(t_struct* tstruct) {
  ...
  f_erl_nif_c_ << indent() << "tuple_t args = read_value<tuple_t>(env, term);" << endl;
  f_erl_nif_c_ << indent() << "check_record(env, args.at(0), \"" << uncapitalize(name) << "\");" << endl;
  f_erl_nif_c_ << indent() << name << " result;" << endl;
  ...
  for(std::size_t i = 0; i < fields.size(); ++i )
  {
    ...
    f_erl_nif_c_ << indent()
      ...
      << "(env, args.at(" << i+1 << "), "
      << "&result, &" << name << "::__set_" << field->get_name() << ");" << endl;
  }
  ...
  f_erl_nif_h_ << indent() << "ERL_NIF_TERM write_value(ErlNifEnv* env, " << name << " const& value);" << endl << endl;
  f_erl_nif_c_ << indent() << "ERL_NIF_TERM write_value(ErlNifEnv* env, " << name << " const& value) {" << endl;

  indent_up();

  f_erl_nif_c_ << indent() << "tuple_t result;" << endl;
  f_erl_nif_c_ << indent() << "result.push_back( write_value(env, atom_t(\"" << uncapitalize(name) << "\")) );" << endl;
  ...
  for(std::size_t i = 0; i < fields.size(); ++i )
  {
    ...
    {
      f_erl_nif_c_ << indent() << "result.push_back( write_value(env, value." << field->get_name() << ") );" << endl;
    }
    else
    {
      f_erl_nif_c_ << indent() << "if (value.__isset." << field->get_name() << ") {" << endl;
      indent_up();
      f_erl_nif_c_ << indent() << "result.push_back( write_value(env, value." << field->get_name() << ") );" << endl;
      indent_down();
      f_erl_nif_c_ << indent() << "} else {" << endl;
      indent_up();
      f_erl_nif_c_ << indent() << "result.push_back( write_value(env, atom_t(\"undefined\")) );" << endl;
      indent_down();
      f_erl_nif_c_ << indent() << "}" << endl;
    }
  }

  f_erl_nif_c_ << indent() << "return write_value(env, result);" << endl;
  indent_down();
